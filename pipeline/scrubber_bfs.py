#!/usr/bin/env python
# This is an unmodified copy of scrubber_bfs.py taken from
...
while (len(G) != 0):
    if (len(bfs_subset) > 0):
        possible_starts = set(G.nodes()).difference(
            bfs_subset)  # avoid to choose starting vertex that is already contained in subset

    else:
        possible_starts = set(G.nodes())
    ...
    if (len(bfs_subset) < subset_size and len(G) > len(
            bfs_subset)):  # the conncected component is smaller than threshold size -> so merge it to next one
        continue
    ...
    os.system(
        'minimap2 -x ava-ont ' + tmp_dir + '/temp_sequences.fa ' + tmp_dir + '/temp_sequences.fa > ' + tmp_dir + '/temp_pwa.paf 2> /dev/null')
    ...
    for node in bfs_subset_center:
        print_corrected_read(node, G.nodes[node]['seq_to_ranges'], G.nodes[node]['illu_to_ranges'],
                             G.nodes[node]['length'])
