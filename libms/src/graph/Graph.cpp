...
auto getVertexMap(std::vector<muchsalsa::graph::Vertex *> const &vertices) {
...
Graph Graph::getSubgraph(std::vector<muchsalsa::graph::Vertex *> const &vertices) {
...
DiGraph DiGraph::getSubgraph(std::vector<muchsalsa::graph::Vertex *> const &vertices) {
  std::vector<std::shared_ptr<Edge>> e;
  e.reserve(getSize());
...
std::vector<muchsalsa::graph::Vertex const *> DiGraph::sortTopologically() const {
  std::vector<muchsalsa::graph::Vertex const *> result;

  std::unordered_map<muchsalsa::graph::Vertex const *, std::size_t> verticesWithNonNullInDegree;
