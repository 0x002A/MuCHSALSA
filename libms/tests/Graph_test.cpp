...
TEST(GraphTest, EdgeCreationTest) {
  auto const spVertex1 = std::make_shared<muchsalsa::graph::Vertex>(2, 0);
  auto const spVertex2 = std::make_shared<muchsalsa::graph::Vertex>(1, 0);

  auto const edge = muchsalsa::graph::Edge(std::make_pair(spVertex1, spVertex2));
...
TEST(GraphTest, InverseOrderTest) {
  auto spVertex1 = std::make_shared<muchsalsa::graph::Vertex>(2, 0);
  auto spVertex2 = std::make_shared<muchsalsa::graph::Vertex>(1, 0);
...
TEST(GraphTest, DirectedGraphTest) {
  ...
  auto graph    = muchsalsa::graph::DiGraph();
  graph.addVertex(spVertex1->getSharedPtr());
  graph.addVertex(spVertex2->getSharedPtr());
  ASSERT_EQ(graph.getOrder(), 2);
...
TEST(GraphTest, EdgeDeletionTest) {
  ...
  auto diGraph = muchsalsa::graph::DiGraph();
...
TEST(GraphTest, SubgraphTest) {
  ...
  auto const subgraphVertices = std::vector<muchsalsa::graph::Vertex *>({v1, v2});
...
TEST(GraphTest, ShortestPathTest) {
  ...
  auto shortestPath =
      muchsalsa::GraphUtil::getShortestPath(muchsalsa::util::make_not_null_and_const(&graph), shortestPathVertices);
  ...
  shortestPath =
      muchsalsa::GraphUtil::getShortestPath(muchsalsa::util::make_not_null_and_const(&diGraph), shortestPathVertices);
