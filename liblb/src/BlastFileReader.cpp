...
BlastFileReader::parseLine(gsl::not_null<const threading::Job*> pJob)
{
  ...
  std::istringstream iss(std::any_cast<std::string>(pJob->getParam(1)));
  std::string token;
  ...
  if (addNode) {
    ...
    auto spVertexMatch = std::make_shared<lazybastard::matching::VertexMatch>(
                           nanoporeRange, illuminaRange, rRatio, direction, matches, thresholdsPassed);
    m_pMatchMap->addVertexMatch(nanoporeID,
                                illuminaID,
                                std::move(spVertexMatch));
  }
